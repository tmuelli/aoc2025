...
char *createCompareString(char *src, size_t srcLen, int repeatCount) {
    if (repeatCount == 0) return NULL;

    // will be freed outside
    char *cmp = (char*) calloc(1, srcLen * repeatCount + 1);

    for (int i = repeatCount; i > 0; i--) {
        strncat(cmp, src, srcLen);
    }
}
...
bool analyzeRange(char *pStart, char *pEnd, long long *pResult) {
    ...
    for (long long i = 0; i <= (llNum2 - llNum1); i++) {
        ...
        // when even search for matching substring pattern for half of length of number string
        // when odd search for matching substring pattern for half of length of number string minus one
        size_t len = strlen(num);
        bool isEven = (len % 2) == 0;
        int cmpLen = ((len - (isEven ? 0 : 1)) / 2);
        bool invalid = false;
        for (int j = 1; j <= cmpLen; j++) {
            char *cmpNum = createCompareString(num, j, (len / j));

            if (strncmp(cmpNum, num, len) == 0) {
                free(cmpNum);
                invalid = true;
                break;
            }
